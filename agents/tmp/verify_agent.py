...
class VerifyAgent:
    ...
    def __init__(self, config):
        self.config = config
        verify_cfg = self.config["agent_endpoints"]["verify"]
        self.model_name = verify_cfg["model_name"]
        self.vllm_endpoint = verify_cfg["endpoint"]
        self.api_key = self.config.get("openai", {}).get("api_key", "")

        self.system_prompt = SYSTEM_PROMPTS.get("verify", "Verify system prompt.")
        self.llm_client = OpenAIClient(api_key=self.api_key, base_url=self.vllm_endpoint)

        def run(self, context: TaskContext) -> None:
            try:
                logging.info("Verify Agent started.")
                ...
                if extract_response:
                    msgs.append(
                        {"role": "assistant", "content": f"Here is a extracted numerical data {extract_response}"})
                ...
                print(f"response from Verify agent: {response}")
                ...
                logging.info("VerifyAgent complete.")
            except Exception as e:
                raise RuntimeError(f"VerifyAgent failed: {e}")
