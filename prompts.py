...
- You are only allowed to operation "+", "-", "*", "/" for the Python code. 
- You are NOT ALLOWED to use "for", "while" loop and Python function such as "def function_name()", "try  ... catch ..." for the python code.
...
You are an expert Python code reviewer. Your task is to evaluate the following Python code for any errors, logical flaws. Provide detailed feedback to help refine and enhance the code.
...
2. **Logical Flaws:**
   - Make sure that the code following the logic.
   - You.
...
#
#     Based on the variables and the question, extract the value from the following relevant chunks:
#     {relevant_chunks}
#     """
#     return prompt

def construct_extractor_prompt(variables: str, relevant_chunks: List[Dict[str, Union[str, float]]], input_question: str) -> str:
    return f"""You are an AI assistant specialized in extracting and analyzing financial data from semi-structured inputs, including text and tables.
    ...
    **Step 1: Identify the Top 4 Potential Candidates**
    - From the provided 'Variables' section (which may include extra variables) and the context, list the top 4 items that are most relevant to answering the question.
    - Rank them in order of relevance (1 being the most relevant).

    **Format your response exactly as follows for Step 1:**
    **Top 4 Potential Candidates:**
    1. [Candidate 1]
    2. [Candidate 2]
    3. [Candidate 3]
    4. [Candidate 4]

    **Step 2: Select the Most Relevant Candidate and Provide Its Values**
    - Based on the candidates you identified in Step 1, select the one candidate that best answers the question.
    - Provide the value for the selected candidate for each year mentioned in the question.

    **Format your response exactly as follows for Step 2:**
    **Selected Candidate and Values:**
    - **Selected Candidate:** [Candidate Name]
    - **Selected Value:**
      - **[Variables 1]:** [Value 1]
      - **[Variables 1]:** [Value 2]
      - **[Variables 1]:** [Value 3]
    - **Conclusion:** [Answer to the question based on the values]. Therefore, the answer is [final option].
    ....(same as for rest of Variables)
        
    Please include both your candidate ranking and the final selection with values in a single response.
    """
...
ACTIONS = {
    ...
    **Example Output in JSON:**
    ...
    """,
    "REASON_ACTION_IDENTIFY_VAR":
    """Identify variables and it's meaning involved in the question.
    **Output in JSON:**
    {
    ...,
